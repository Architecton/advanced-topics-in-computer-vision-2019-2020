...
def kalman_step(A, C, Q, R, y, x, V):
    ...
    # Get dynamic model distribution mean by multiplying the prior 
    # mean (result from previous step) with transition matrix.
    xpred = np.matmul(A, x)

    # Get variance of dynamic model distribution.
    Vpred = np.matmul(np.matmul(A, V), A.transpose()) + Q
    
    # Get error of observation.
    e = y - np.matmul(C, xpred)

    # Se Kalman filter equations.
    ...
    Vnew = np.matmul(np.matmul((np.eye(ss) - np.matmul(K, C)), Vpred), (np.eye(ss) - np.matmul(K, C)).transpose()) + np.matmul(K, np.matmul(C, K.transpose()))
    VVnew = np.matmul(np.matmul((np.eye(ss) - np.matmul(K, C)), A), V)
    
    # Return mean of posterior, variance of posterior, log-likelihood of innovation and covaiance of joint
    # probabilities of current and next state given sequence of measurements.
