...
from bpf_tracker import BPFTracker, BPFParams
...
# Initialize tracker.
parameters = BPFParams()
tracker = BPFTracker(parameters)
...
while frame_idx < sequence.length():
    ...
    if frame_idx == init_frame:
        ...
        t_ = time.time()
        tracker.initialize(img, sequence.get_annotation(frame_idx, type='rectangle'), sequence)
    ...
    sequence.draw_region(img, np.array(predicted_bbox), (0, 0, 255), 2)
    if True and particles is not None:
        for idx, particle in enumerate(particles[0]):
            sequence.draw_region(img, list(particle[:2] + [particle[2]/2, particle[3]/2]) + [10, 10] , (255, 0, 0), 1)
