...
class NCCTracker(Tracker):
    ...
    def initialize(self, image, region):
        ...
        # Set tracker name.
        self.name = "ncc-tracker"
       
        # This conversion is normally done using convert_region function
        # in sequence_utils.py
        if len(region) == 8:
        ...
        # Left and right sides of 'width' interval.
        left = max(region[0], 0)
        right = min(region[0] + region[2], image.shape[1] - 1)

        # Top and bottom sides of 'height' interval.
        top = max(region[1], 0)
