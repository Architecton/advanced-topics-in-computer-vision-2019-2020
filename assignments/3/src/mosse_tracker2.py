...
class MosseTracker(Tracker):
    ...
    def initialize(self, img, region):
        ...
        gauss_im = self._gauss_image(img_init, self.init_gt, 20)
        ...
        # Extract region of interest (feature patch) from initial frame.
        img_ext = img_init[self.init_gt[1]:self.init_gt[1]+self.init_gt[3], self.init_gt[0]:self.init_gt[0]+self.init_gt[2]]
    ...
    def track(self, img):
        ...
        # Construct filter.
        filt = self.filt_num / self.filt_denom

        # Preprocess image - normalize and apply Hanning window.
        img_prep = self._preprocess(img_gray[self.inb_pos[1]:self.inb_pos[3], self.inb_pos[0]:self.inb_pos[2]])
        ...
        self.filt_num =  (1 - self.parameters.alpha) * self.filt_num + self.parameters.alpha * (self.gauss_fd * np.conjugate(np.fft.fft2(img_mov)))
        self.filt_denom = (1 - self.parameters.alpha) * self.filt_denom + self.parameters.alpha * (np.fft.fft2(img_mov) * np.conjugate(np.fft.fft2(img_mov)))
...
class MosseParams:
    ...
    def __init__(self, lmbd=0.1, alpha=0.1, training_iter=10, rotate=False):
        self.lmbd = lmbd
        self.alpha = alpha
